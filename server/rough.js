...
export const fieldValidationsAndExtraSpaceCleanUp = (fieldWithValidationType)=>{
      if(fieldWithValidationType.type==="name")
      {
        if(validators.name(fieldWithValidationType.field)){
          const valueWithNoSpace = validators.noExtraSpaces(fieldWithValidationType.field)
          return valueWithNoSpace.replace( /\b\w/g, (matched)=>matched.toUpperCase()).trim()
        }
      } 
      else{
...
const str = "this         is   so many "
const type="name"
console.log(str)
const result = fieldValidationsAndExtraSpaceCleanUp({type,field:str})
console.log(result)
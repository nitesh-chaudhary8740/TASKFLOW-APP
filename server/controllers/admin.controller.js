...
req.body.password=password
  try {
  await  sendEmail(email,"MY APP",mailData(fullName,userName,password),"Welcome to app")
  } catch (error) {
    console.log(error)
  }
  const emp =await Employee.create({
    userName,fullName,email,password,designation
  })
...
const fetchedAdmin = await Admin.findOne({$or:[{name:userNameOrEmail},{email:userNameOrEmail}]})
if(!fetchedAdmin) {
...
const fetchAdminMetaData = async(req,res)=>{
  ...
  const pendingTasks = await Task.find({status:"pending"})
  const tasks = await Task.find()
  res.status(200).send({totalEmployees:employees.length,totalPendingTasks:pendingTasks.length,totalTasks:tasks.length});
}
...
module.exports = {employeeRegistration,adminLogin,taskCreation,fetchAllEmployees,fetchAllTasks,fetchAllPendingTasks,assignTask,fetchAdminMetaData}